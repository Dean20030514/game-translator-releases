...
- 一旦本轮 fix path 选定并实施完成，本文件应：
  - (a) 移到 `_archive/AUDIT_R60.md` 作为历史记录（与 r57 旧版本相同处理）
  - (b) 或继续重写为下一轮新审计的容器（"AUDIT 永久入口"模式）
- 当前**未 commit**——选择 fix 范围后再决定是否一起 commit
- 与本轮（r60）的 EVOLUTION 滚动归档（hard contract #15）**独立处理**——归档在 docs sync 阶段执行，与 audit fix 不耦合
